...
public class PokerFace extends AbstractNegotiationParty {
	...
	private double[] last_rounds_time; //array to store time of last rounds, used in moving average
	private final int MA_SIZE = 10;
	private double average_time = 0.001; // average time taken for a round (if time is continuous) to avoid infinity, initial non 0
	
	private Random random;
	
	
	public PokerFace(UtilitySpace utilitySpace,
	...
	public Action chooseAction(List<Class> validActions) {
		...
		if (!validActions.contains(Accept.class) || (last_utility < threshold && (time_left > 1 || first_to_offer == 1))) {
			if (((timeline instanceof ContinuousTimeline ? current_time : rounds) / total_time) < RANDOM_WALKER_TRESHOLD) {
				...
				if ((int)time_left<3) {
					...
					List<Entry<Pair<Integer,Value>, Integer>> pair_frequency = opponent_bid_list.getMostFrequentIssueValues(senders.get(0));
					List<Entry<Pair<Integer,Value>, Double>> weighted_list = opponent_bid_list.weightIssueValues(pair_frequency);
...
//				System.out.println("Average Bid, Utility:"+utilitySpace.getUtility(average_bid)+" and "+actions_left+" steps to go.");
	...
	private Bid generateConcedeBid(BigInteger step, Bid bid, List<Entry<Pair<Integer,Value>, Double>> ordered_pair, int issue_count){
...
//		System.out.println("Random bid: "+bids.get(index).toString());
	...
	private void updateTime(){
		...
		// using an moving average prevents averaging over the full timeline where
