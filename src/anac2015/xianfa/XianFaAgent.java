...
public class XianFaAgent extends AbstractNegotiationParty {
	...
	public XianFaAgent(UtilitySpace utilitySpace,
				  Map<DeadlineType, Object> deadlines,
				  Timeline timeline,
				  long randomSeed) {
		// Make sure that this constructor calls it's parent.
		super(utilitySpace, deadlines, timeline, randomSeed);
		//if (utilitySpace.getDomain().getNumberOfPossibleBids() > 200000) attitude = 2;
		//else attitude = 0;
		try {
			initTree();
			calculate();
			initBS();
			initOpp();
			if (debug) stat = new Statistician(this);
		} catch (Exception e) {
			e.printStackTrace();
		}		
	}
	...
	public Action chooseAction(List<Class> validActions) {
		...
		try { 
			if(firstRound) action = chooseBid();
			
			else {				
				if (acceptable()) {
					if (debug) stat.log();
					if (aU > utilitySpace.getUtility(partnerBid)) aU = utilitySpace.getUtility(partnerBid);
					act = 1;
					action = new Accept();
					...
					if (opponentABest != null) {
						if (utilitySpace.getUtility(opponentABest) > resValue) {
							if (utilitySpace.getUtility(opponentABest) > (1.03 * utilitySpace.getUtility(partnerBid))) {
								stat_offerHistBest++;
								if (oU > utilitySpace.getUtility(opponentABest)) oU = utilitySpace.getUtility(opponentABest);
	...
	private Action chooseBid() {
		...
		try {
			if (oU > utilitySpace.getUtility(nextBid)) oU = utilitySpace.getUtility(nextBid);
	...
	private void updateSelf() {
		...
		if (!opponentA.equals("") && !opponentB.equals("")) {
			...
			if (rounds % 200 == 0) {
				...
				threshold = threshold - ((Math.max(1-sRateA, 1-sRateB) * 0.0116) * ((1-t) * (1-t) * (1/discount)));
			}
			//printConsole();
		}
		if (attitude == 2) return;
		if (attitude == 1) return;
	...
	public void receiveMessage(Object sender, Action action) {
		...
		if (!sender.toString().equals("Protocol") && firstRound) {
			if (!(opponentA.equals("") || opponentB.equals(""))) firstRound = false;
			if (!myTurnPrev) {
				if (!opponentA.equals("")) {
					opponentB = sender.toString();
				}
			} else opponentA = sender.toString();
		}
		...
		if (action instanceof Offer) {
			...
			if (myTurnPrev) {
				...
				try {
					if (opponentABest == null) opponentABest = new Bid(partnerBid);
		...
		if (myTurnPrev) myTurnPrev = false;
	}
	...
	public void initTree() throws Exception {
		...
		for (int curDepth=0; curDepth<maxDepth; curDepth++) { // for this depth
			tree.addNewDepth(); // initialize new level
			if (utilitySpace.getDomain().getIssue(curDepth).getType() == ISSUETYPE.DISCRETE) {
				id = (IssueDiscrete) utilitySpace.getDomain().getIssue(curDepth);
				ed = (EvaluatorDiscrete) utilitySpace.getEvaluator(curDepth+1);
				for (int curMember=0; curMember<tree.getSizeOfLevel(curDepth); curMember++) { // for all nodes in this depth
					for (int i=0; i<id.getNumberOfValues(); i++) { // for all evaluations for next issue below this depth
						Node node = new Node(id.getNumber(), id.getValue(i), curDepth, ed.getEvaluation(id.getValue(i)));
	...
	public Bid offerBid() throws Exception {
		...
		if (myOfferedBids.size() < supposedNumber) {
			...
			for (int i=0; i<shortList.size(); i++) {
				if (utilitySpace.getUtility(shortList.get(i)) > threshold && !myOfferedBids.contains(shortList.get(i))) {
	...
	public Bid getBid(int position) throws Exception {
		...
		values.put(tree.getMemberInLevel(maxDepth, position).getNumber(), tree.getMemberInLevel(maxDepth, position).getValue());
