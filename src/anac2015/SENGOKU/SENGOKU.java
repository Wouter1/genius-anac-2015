...
public class SENGOKU extends AbstractNegotiationParty {
	...
	//コンストラクター　初期化
	public SENGOKU(UtilitySpace utilitySpace, Map<DeadlineType, Object> deadlines, Timeline timeline, long randomSeed) throws Exception {
	...
	public Action chooseAction(List<Class> validActions) {
		if(isPrinting){System.out.println("-----------------MyTern----------------------"+timeline.getTime()*180);}
		...
		if (time >0.05){
			try {
				if(isPrinting){System.out.println("有効値："+utilitySpace.getUtility(offeredBid));}
			} catch (Exception e) {
		...
		if (negotiatingInfo.lastFlag){
			if(isPrinting){System.out.println("last-------------------------------------------");}
			return lastAction();
		}
		
		//アクセプトの処理
		if(validActions.contains(Accept.class) && strategy.selectAccept(offeredBid, time)){ 
			if(isPrinting){System.out.println("MyAction:Accept");}
			return new Accept(); 
		}
		...
		if(strategy.selectEndNegotiation(time) || strategy.endNegotieitionFlag ){ 
			//System.out.println("flag:"+strategy.endNegotieitionFlag +"タイム"+strategy.selectEndNegotiation(time));
	...
	public Action lastAction() {
		if(isPrinting){System.out.println("ラストリストの数"+negotiatingInfo.getLastOfferBidNum());}
		
		if (negotiatingInfo.getLastOfferBidNum() == 0){ //最後のオファー用リストがなくなったとき
			if(isPrinting){System.out.println("カラリストでアクセプト");}
			return new Accept(); 
		}else{ //
			...
			if (offerUtil > lastUtil){
				if(isPrinting){System.out.println("lastAccept");}
				return new Accept(); 
			}else {
				if(isPrinting){System.out.println("lastOffer");}
	...
	/*
	private Bid noCooperateOffer(){
		ArrayList<Bid> MyBidHistory = negotiatingInfo.getMyBidHistory();
		while(true){
			int flag = 0;
			Bid offerBid = bidSearch.getBid(generateRandomBid(), strategy.myThreshold);
			for (Bid mybid:MyBidHistory){
				if(offerBid.equals(mybid) ){
					flag = 1;
					continue;
				}
			}
			if(flag == 1){
				strategy.myThreshold = strategy.myThreshold -0.0005;
			}else{
				negotiatingInfo.updateMyBidHistory(offerBid);
				return offerBid;
			}
	...
	*/
	...
	// senderが相手　相手名
	//　Actionはどんなアクションでオファーのときは中身もわかる！
	//　絶えず相手の情報がわかる　なにもしているのか！
	public void receiveMessage(Object sender, Action action) {
		...
		if (action != null ) {
			...
			if(action instanceof Accept){
		    	...
		    	if(!negotiatingInfo.getOpponents().contains(sender)){// 初出の交渉者は初期化 相手のリストにないから最初いれていく！
		    		negotiatingInfo.initOpponent(sender);   //negotiatingInfoのopperatersインスタンスに保存している
		    		} 
			}
			
			//相手がオファーを出した時
			else if(action instanceof Offer) {
				if(!negotiatingInfo.getOpponents().contains(sender)){ negotiatingInfo.initOpponent(sender); } // 初出の交渉者は初期化
				offeredBid = ((Offer) action).getBid(); // 提案された合意案候補
				try { negotiatingInfo.updateInfo(sender, offeredBid); } // 交渉情報を更新
				catch (Exception e) { 
				...
				int opponentsNum = Integer.parseInt(obj.toString().replaceAll("[^0-9]","")); //相手のナンバーを取る
