...
public class negotiatingInfo {		
    ...
    public void updateNegotiatingInfo(Object sender, Bid offeredBid) throws Exception {
    	...
    	if(!lastFlag){
	    	...
	    	}else if (utilitySpace.getUtility(LastOfferBidHistory.get(LastOfferBidHistory.size()-1)) < util){
	    		LastOfferBidHistory.add(offeredBid);
	    	}
    	}
    ...
    // アクセプト率をリセット
    public void resetAcceptRate() { acceptRate=0.0;}
    
    // 自分が何をするか　オファーのとき
    public void myActionOffer() { ActionFlag  = 0;}
    
    // 自分が何をするか　アクセプトの時
    public void myActionAccept() { ActionFlag  = 1;}	
    ...
    // 最新の平均
    public double getNewAverage(Object sender) { return newAverage.get(sender); }

    // 最新の分散
    public double getNewVariancer(Object sender) { return newVariance.get(sender); }
    
    // 平均
    public double getAllAverage(double m) { return allAverage; }

    // 分散
    public double getAllVariancer(double v) { return allVariance; }
    
    // 標準偏差
    public double getStandardDeviation(Object sender) { return opponentsStandardDeviation.get(sender); }
 
    // 新しい標準偏差
    public double getNewStandardDeviation(Object sender) { return newStandardDeviation.get(sender); }
    
    // 相手の協力状態を返す
    public ArrayList<Double> getBidValue(Object sender) { return BidValue.get(sender); }
    
    // 相手の協力状態を更新
    public void updateBidValue(Object sender , double util) { BidValue.get(sender).add(util); }
    ...
    // 相手の提案履歴を返す
    public ArrayList<Bid> getPartnerBid(Object sender) { return opponentsBidHistory.get(sender); }
    
    //相手の協力情報を更新する　協力ならtrue
    public void updateopponentsCooperateHistory(Object sender , boolean state){opponentsCooperateHistory.get(sender).add(state);}
    
    //相手の協力情報をリストで返す
    public ArrayList<Boolean> getopponentsCooperateHistory(Object sender){return opponentsCooperateHistory.get(sender);}
    ...
    //最後のビットのリストのサイズを返す
    public int getLastOfferBidNum (){return LastOfferBidHistory.size();}
    ...
    // 自分の提案履歴を返す
    public ArrayList<Bid> getMyBidHistory() { return MyBidHistory; }
    
    //オファーしている相手を返す
    public Object getOfferPlayer() {return OfferPlayer;}
    
    // 新しい提案の重複数
    public double getNewSumNum(Object sender) { return newSumNum.get(sender); }

    // 新しいアクセプト数
    public double getNewAccept(Object sender) { return newAccept.get(sender); }
    ...
    // アクセプト数を返す
    public int getAccept() { return acceptNum; }
    
    // アクセプト数を返す
    public double getAcceptRate() { return acceptRate; }
    
    // 相手の提案の有効値の最大を返す
    public double getmaxUtil(Object sender) { return maxUtil.get(sender); }
	...
	public void laststrategy (double time){
		...
		//System.out.println(LastOfferBidHistory.size() + "-----list"+LastOfferBidHistory);
