...
{	
	...
	private ArrayList<Bid> MyBidHistory = new ArrayList<>();; // 提案履歴 自分！
	...
	public Bid shiftBidSerch(Bid baseBid,double threshold) {
		...
		for(int i =0 ; i<backNum ; i++){
			for (int t = 0 ; t < playerNum ; t++){
				ArrayList<Bid> bidHistry = opponentsBidHistory.get(opponents.get(t)); 
				int n = bidHistry.size();
				//System.out.println(bidHistry);
				if (i+1 > n) {return getBid(baseBid ,threshold);}
				...
				for (Bid currentbid:bidArray) {
					...
					if(isPrinting){System.out.println("置換中:"+util +"目標閾値:"+threshold);}
					if (util > threshold){
						for (Bid mybid:MyBidHistory){//今までのここで使って提案と同じのだったらオファーしない
							if(currentbid.equals(mybid) ){
								...
								MyBidHistory.remove(MyBidHistory.indexOf(currentbid));
								if(isPrinting){System.out.println("同じのでてきたーーーーーーーーーーーーーーーー");}
								return getBid(baseBid ,threshold);
							}
						}
						
						//今までのオファーで同じのはない
						if(isPrinting){System.out.println("置換したビットをオファーする");}
