...
public class strategy {
	...
	public strategy(UtilitySpace utilitySpace, negotiatingInfo negotiatingInfo , bidSearch bidSeach) {		
	...
	public void updateThreshold(double num) { myThreshold = num; }
	 // 自身のラウンド数を返す
	...
	public ArrayList<Double> getCooperateRate () {
		...
		for (Object sender:opponents){
			...
			ArrayList<Boolean> states = negotiatingInfo.getopponentsCooperateHistory(sender);
	...
	public double cooperateThreshold(double time) {
		...
		double fThreshold = cooperateRate * maxThreshold + (1 - cooperateRate) * minThreshold; //最終的な効用値を協力レートから算出
		double dffThreshold = fThreshold *df;  //最終の結果に割引効用を適用する。
		double rvUtil = rv * Math.pow(df,time);  //現在もらうことができる留保価格を算出する。
		
		double hardThreshold = fThreshold + (1 -fThreshold) * (1 - Math.pow(time,1/e)); //実際の使用する閾値　ファティマの式を線形でいれる
		
		if (dffThreshold < rvUtil) { //エンドネゴシエーション
			if(isPrinting){System.out.println("最後の閾値にリザ："+dffThreshold + "リザベーションバリュー：" + rvUtil);}
			double harddffThreshold = dffThreshold + (1 -dffThreshold) * (1 - Math.pow(time,0.5)); //最後の予想値にファティマで落としていく
			if(isPrinting){System.out.println("線形で最後のリザにおとしていくときの閾値:"+harddffThreshold);}
	...
	/*
		if (df == 0.0){  //割引効用がないとき
			//期待予想値にファティマの式を代入する。
			hardThreshold = fThreshold + (1 -fThreshold) * (1 - Math.pow(time,1/e));
			//System.out.println("協力閾値"+hardThreshold);
		}else { //割引効用があるとき
			hardThreshold = fThreshold / Math.pow(df,time);
			double firstHardthreshold = dffThreshold + (1 - dffThreshold) * (1 - Math.pow(time,1/e));
			if (hardThreshold < firstHardthreshold) {
				hardThreshold = firstHardthreshold;
			}
		}
	*/
		
		if(isPrinting){System.out.println("今の協力値"+hardThreshold + "最後の予想値" + fThreshold);}
	...
	private double thresholdUpRate (Object sender , int num)  {
		...
		double rateSum = 0;
		if (List.size() < num){return 0;} //閾値の配列に要素がないとき
	...
	private double hardThreshold (double threshold,double time) {
		...
		double hardthreshold = threshold + (1 - threshold) * (1 - Math.pow(useTime,1/e));
		return hardthreshold;
	}
	...
	private double strongThreshold() {
		...
		/*
		ArrayList<Object> opponents =  negotiatingInfo.getOpponents();
		if (negotiatingInfo.getRound() < 3){ //相手の情報がないとき
			return 1.0 - (Math.random() * 0.1);
		}else{
			double sdAll=0.0; //標準偏差
	...
	private void eSet(double threshold,double time){
		System.out.println("--------set------------" + time + "------"+(double)20/180 );
		if(time<(double)20/180){return;}//最初はデータがないからできない 
		...
		int flag = 0;
		double myRate = (hardThreshold(threshold,time) - hardThreshold(threshold,time + (double)1/180)) / hardThreshold(threshold,time);
		...
		while(true){
			...
			myRate = (hardThreshold(threshold,time) - hardThreshold(threshold,time + 1/180)) / hardThreshold(threshold,time);
			if(myRate>rateMax){ //自分の方が下げている
				if (flag == 1){return;}
				e= e - 0.1; 
			}else{ //相手の方が下げている
