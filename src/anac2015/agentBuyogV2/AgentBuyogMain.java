...
public class AgentBuyogMain extends AbstractNegotiationParty{
	...
	private static final double alphaDefault = 0, betaDefault = 0, issueWeightsConstant = 0.3, issueValuesConstant = 100, minimumHistorySize = 50, learningTimeController = 1.3; //Discount is added to the learningTimeController. So it's actually learningTimeController + discount
	...
	public AgentBuyogMain(UtilitySpace utilitySpace,
	...
	public Action chooseAction(List<Class> possibleActions) {
		...
		double minimumPoint = utilitySpace.getDiscountFactor()*0.7 + utilitySpace.getReservationValueUndiscounted()*0.3;
		...
		if(infoA != null && infoB !=null && infoA.getAgentDifficulty() != null && infoB.getAgentDifficulty() !=null){
		...
		double acceptanceThreshold = minimumPoint + (1 - minimumPoint)*(1-Math.pow(timeline.getTime(), timeConcessionController));
		...
		if(possibleActions.contains(Accept.class)){
			if(mostRecentBidsUtility >= acceptanceThreshold && mostRecentBidsUtility >= bestAgreeableBidsUtility && remainingRounds <= 3){
				return new Accept();
			}	
		}
		...
		if(possibleActions.contains(Accept.class)){
			if(mostRecentBidsUtility >= acceptanceThreshold && mostRecentBidsUtility >= bestAgreeableBidsUtility && mostRecentBidsUtility >= bestBid.getMyUndiscountedUtil()){
				return new Accept();
			}	
		}
	...
	private Double getDistance(BidDetails bid, Double infoAKalai,
			Double infoBKalai) {
		try {
			return Math.sqrt((1-infoA.getAgentDifficulty())*Math.pow(infoA.getOpponentUtilitySpace().getUtility(bid.getBid()) - infoAKalai, 2) + (1-infoB.getAgentDifficulty())*Math.pow(infoB.getOpponentUtilitySpace().getUtility(bid.getBid()) - infoBKalai, 2));
		} catch (Exception e) {
			e.printStackTrace();
		}
		return 0D;
	...
	public void receiveMessage(Object sender, Action action){
		...
		if(sender.toString().equalsIgnoreCase("Protocol") || !((action instanceof Offer) || (action instanceof Accept))){
			return;
		}
	...
	private void updateOpponentModel(OpponentInfo senderInfo) {
		...
		if(senderInfo.getAgentBidHistory().size()>=minimumHistorySize){
			LearningFunction function = new LearningFunction(senderInfo.getAgentBidHistory().getHistory().get(0).getMyUndiscountedUtil());
		...
		UtilitySpace opponentUtilitySpace = senderInfo.getOpponentUtilitySpace();
		...
		double goldenValue = issueWeightsConstant*(1-(Math.pow(timeline.getTime(), learningTimeController + utilitySpace.getDiscountFactor())))/numberOfIssues;
		...
		for(Integer issueNumber: changed.keySet()){
			if(!changed.get(issueNumber) && opponentUtilitySpace.getWeight(issueNumber)<maximumWeight){
				opponentUtilitySpace.setWeight(opponentUtilitySpace.getDomain().getObjective(issueNumber),(opponentUtilitySpace.getWeight(issueNumber) + goldenValue)/ totalSum);
			}
			else{
				opponentUtilitySpace.setWeight(opponentUtilitySpace.getDomain().getObjective(issueNumber), opponentUtilitySpace.getWeight(issueNumber)/ totalSum);
			}
		}
		...
		try{
			for(Entry<Objective, Evaluator> issueEvaluatorEntry: opponentUtilitySpace.getEvaluators()){
				if(issueEvaluatorEntry.getKey() instanceof IssueDiscrete){
					((EvaluatorDiscrete)issueEvaluatorEntry.getValue()).setEvaluation(opponentsLatestBid.getBid().getValue(((IssueDiscrete)issueEvaluatorEntry.getKey()).getNumber()), (int)(issueValuesConstant*(1-Math.pow(timeline.getTime(), learningTimeController + utilitySpace.getDiscountFactor())) + ((EvaluatorDiscrete)issueEvaluatorEntry.getValue()).getEvaluationNotNormalized((ValueDiscrete)opponentsLatestBid.getBid().getValue(((IssueDiscrete)issueEvaluatorEntry.getKey()).getNumber()))));
				}
					...
					int opponentsLatestValueForIssueAsInteger = ((ValueInteger)opponentsLatestValueForIssue).getValue();
					...
					if(midPoint>opponentsLatestValueForIssueAsInteger){
						...
						double total=1;
						double newLowEndEvaluation = ((EvaluatorInteger) issueEvaluatorEntry.getValue()).getEvaluation(lowerBound)+(issueValuesConstant/10000)*normalizedDistanceFromMidPoint*(1-Math.pow(timeline.getTime(), learningTimeController+utilitySpace.getDiscountFactor()));
						double highEndEvaluation = ((EvaluatorInteger) issueEvaluatorEntry.getValue()).getEvaluation(upperBound);
						
						if(newLowEndEvaluation > 1){
							total = newLowEndEvaluation+highEndEvaluation;
						}
						
						((EvaluatorInteger) issueEvaluatorEntry.getValue()).setLinearFunction(newLowEndEvaluation/total, highEndEvaluation/total);
					}
					else{
						double distanceFromMidPoint = opponentsLatestValueForIssueAsInteger - midPoint + 1; //because midPoint is included and I don't want a 0 value. 
						double normalizedDistanceFromMidPoint = distanceFromMidPoint/(upperBound-midPoint+1);
						
						
						double total=1;
						double newHighEndEvaluation = ((EvaluatorInteger) issueEvaluatorEntry.getValue()).getEvaluation(upperBound)+(issueValuesConstant/10000)*normalizedDistanceFromMidPoint*(1-Math.pow(timeline.getTime(), learningTimeController+utilitySpace.getDiscountFactor()));
						double lowEndEvaluation = ((EvaluatorInteger) issueEvaluatorEntry.getValue()).getEvaluation(lowerBound);
						
						if(newHighEndEvaluation > 1){
							total = newHighEndEvaluation+lowEndEvaluation;
						}
						
						((EvaluatorInteger) issueEvaluatorEntry.getValue()).setLinearFunction(lowEndEvaluation/total, newHighEndEvaluation/total);
					}					
				}
			}
		}
...
//			UNCOMMENT THE FOLLOWING LINES IF THE ESTIMATED KALAI POINT IS A BIT TOO CONSERVATIVE
			...
			if(kalaiPoint > senderInfo.getBestBids().getBestBidDetails().getMyUndiscountedUtil()){
				...
				senderInfo.setDomainCompetitiveness(senderInfo.getBestBids().getBestBidDetails().getMyUndiscountedUtil());
			}
			...
			if(senderInfo.getAgentBidHistory().size()>=minimumHistorySize){
				...
				double agentDifficulty = (1-domainWeight)*senderInfo.getLeniency() + domainWeight*senderInfo.getDomainCompetitiveness();
	...
	private HashMap<Integer, Boolean> determineDifference(OpponentInfo senderInfo, BidDetails first, BidDetails second) {
		...
		try {
			for (Issue i : senderInfo.getOpponentUtilitySpace().getDomain().getIssues()) {
				if(i instanceof IssueDiscrete){
					changed.put(i.getNumber(), (((ValueDiscrete) first.getBid().getValue(i.getNumber())).equals((ValueDiscrete) second.getBid().getValue(i.getNumber()))) ? false : true);
				}
				else if(i instanceof IssueInteger){
					changed.put(i.getNumber(), (((ValueInteger) first.getBid().getValue(i.getNumber())).equals((ValueInteger) second.getBid().getValue(i.getNumber()))) ? false : true);
				}
			}
		} catch (Exception ex) {
			ex.printStackTrace();
	...
	private void updateOpponentBidHistory(OpponentInfo opponent, Bid bid) {
		...
		try{
			...
			if(opponent.getBestBid()==null || utilitySpace.getUtility(bid) >= utilitySpace.getUtility(opponent.getBestBid())){
				opponent.setBestBid(bid);
				opponent.getBestBids().add(new BidDetails(bid, utilitySpace.getUtility(bid), timeline.getTime()));
			}
			else{
				opponent.getBestBids().add(new BidDetails(opponent.getBestBid(), utilitySpace.getUtility(opponent.getBestBid()), timeline.getTime()));
