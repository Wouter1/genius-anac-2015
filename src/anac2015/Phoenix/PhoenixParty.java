...
public class PhoenixParty extends AbstractNegotiationParty
{
    ...
    public PhoenixParty(UtilitySpace utilitySpace, Map<DeadlineType, Object> deadlines, Timeline timeline, long randomSeed)
    {
    ...
    public void init()
    {
        try {
            ...
            if(utilitySpace.getDomain().getNumberOfPossibleBids()<50){
                ...
                utilityDivisionSize = Math.min((int) (utilitySpace.getDomain().getNumberOfPossibleBids()/15),utilityDivisionSize);
            }
...
//                    {Math.log(0.1)}
    ...
    {
        if(debug)
            return new AgentID(getName()+"|"+Double.toString(uTargetForBidding)+"|"+Double.toString(uTargetForAcceptance)+"|"+"@" +hashCode());
                ...
                if ((myLastBid == null) || (opponentPreviousBid1 == null)|| (opponentPreviousBid2 == null)) {
                    ...
                    if (opponentPreviousBidUtility>=uTargetForBidding
                        ...
                        if(theta>0&&((currentTime-myLastBidTime)*timeline.getTotalTime()>opponentNoOfferTime
                                ||((domain.getNumberOfPossibleBids()<100&&currentTime>delta*(1.0-theta+bestReceivedBidUtility2))
                                &&(theta>=bestReceivedBidUtility2))
                                &&theta>=0.4*(maxBidUtility-utilitySpace.getUtility(opponentFirstBid2))+utilitySpace.getUtility(opponentFirstBid2))){
                ...
                if (currentTime>=endTime&&domain.getNumberOfPossibleBids()>5){
                    if(opponentPreviousBidUtility>theta){
                        if(theta==0
                                ...
                                ||myLastBidUtility-opponentPreviousBidUtility<(maxBidUtility-minBidUtility)/4){
                            currentAction = new Accept(getPartyId());
                            System.out.println("Accccccccccccccccccccccccccccccccccc");
                        }
                    }
...
//            System.out.println(ex.getMessage());
    ...
    double gamma = 1.0;                                       // ratio of new to all counter-offers over past ten intervals
        ...
        try{
            ...
            if(utilityDivisionSize>30){
                if(rand.nextInt(12)>10){
                    if(indexToFindBids>0)indexToFindBids--;
                }
                ...
                if(domain.getNumberOfPossibleBids()<1000){
                    if(rand.nextInt(10)>=3+delta*Math.pow(6,1-getTime())){
                        indexToFindBids+=rand.nextInt(Math.min(utilityDivisionSize-1-indexToFindBids,Math.abs(indexToRandBids-indexToFindBids)));
                    }
                }else{
            ...
            while(tempBL.size()==0){
                ...
                if(indexToFindBids==utilityDivisionSize-1)break;
            }
...
//                        System.out.println("f["+in+","+j+"]="+freqFunc[in][j]);
    ...
    double opponentConcession()
    {
        ...
        /*
        *
        * modelConcess > 1 : opponent is conceding
        * modelConcess < 1 : opponent is exploiting
        *
        * */
        return modelConcess;
    }
    ...
    private void getTargetUtility() throws Exception
    {
        ...
        double baseValue = (Math.max(Elow, Edelta)+Edelta/uncertainty+uncertainty*Elow)/(1.0+1/uncertainty+uncertainty);
        double diffToElow = 1.2*Math.abs(uTargetForBidding-baseValue);
        if(delta<=1){
            double x=2*(1-getTime())*delta,y=delta,z=(2*getTime())/delta;
            double uTargetForBiddingF1 = diffToElow*Math.pow(5,-(Math.pow(getTime(),0.6)*(2.3-firstAcceptTime))*Math.pow(2-delta,getTime()));
            double uTargetForBiddingF2 = diffToElow*(1/((Math.pow((2*getTime())*(2.3-firstAcceptTime),delta+theta/3.0)+0.05)*20)+0.4*delta);
            double uTargetForBiddingF3 = diffToElow*(1/((Math.log((20.0/(delta+theta/3.0))*getTime()*(2.3-firstAcceptTime)+1.1))*10)+0.3*delta);
            uTargetForBidding = baseValue + (z*uTargetForBiddingF3+y*uTargetForBiddingF2+x*uTargetForBiddingF1)/(x+y+z);
        }
...
//            System.out.println("Predict error: "+ex.getMessage());
    ...
    private double discountedExpectation(double time)
    {
        ...
        double predictedUtility = (uTargetForAcceptance*Math.pow(0.5,getTime())*(2+w)
                ...
                +2*w*meanOfOpponentUtilities)
                    /(2*w+Math.pow(0.5,getTime())*(2+w)+Math.pow(0.5,1-getTime())*3*w*(1.0/PredictUncertainty(time)));
    ...
    {
        double uFIrst = Math.max(utilitySpace.getUtility(opponentFirstBid1), utilitySpace.getUtility(opponentFirstBid2));
        ...
        try{
            ...
            if(getTime()>10*trainingTimeSeperator&&getTime()<0.3){
                ...
                double disToPoint = Math.abs(R-comparePoint)/
                        Math.abs(maxBidUtility - Math.min(utilitySpace.getUtility(opponentFirstBid1),
    ...
    private void recordOffer(Offer offerToRecord) throws Exception
    {
        ...
        if(opponentFirstBid1!=null&&opponentBestOfferBid2!=null){
            compaction2=1/Math.abs(maxBidUtility - Math.min(utilitySpace.getUtility(opponentFirstBid1),
                    utilitySpace.getUtility(opponentFirstBid2)));
        }
        ...
        if(countOpp==lastCountOpp+2) {
            (saveInTermsOfTime[currentTimeIndex]).add(Math.min(opponentPreviousBidUtility1,opponentPreviousBidUtility2));
