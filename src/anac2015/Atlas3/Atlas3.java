...
public class Atlas3 extends AbstractNegotiationParty {
	 ...
	 * @throws Exception 
	 */
	public Atlas3(UtilitySpace utilitySpace, Map<DeadlineType, Object> deadlines, Timeline timeline, long randomSeed) throws Exception {
		// Make sure that this constructor calls it's parent.
		super(utilitySpace, deadlines, timeline, randomSeed);
		
		if(isPrinting){	System.out.println("*** Atlas3 v1.0 ***"); }
	...
	public Action chooseFinalAction(Bid offeredBid, ArrayList<Bid> CList) throws Exception {
		...
		if(CList_index >= CList.size()){
				if(offeredBid_util >= rv){ return new Accept(); } // 遡行を行っても合意が失敗する場合，Acceptする
		...
		}else if(offeredBid_util > rv){ return new Accept(); }
	...
	public void receiveMessage(Object sender, Action action) {
		...
		if (action != null) {
			...
			if(supporter_num == negotiatingInfo.getNegotiatorNum()-1){
				if(offeredBid != null){
					try { negotiatingInfo.updatePBList(offeredBid); } 
					catch (Exception e) {
