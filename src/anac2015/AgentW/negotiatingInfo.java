...
public class negotiatingInfo {		
    ...
    public double getfirst(Object sender) { try {
		return utilitySpace.getUtility(opponentsBidHistory.get(sender).get(0));
	} catch (Exception e) {
		return 0.0;
		}
    }
    ...
    public double getlate(Object sender) { try {
		return utilitySpace.getUtility(opponentsBidHistory.get(sender).get(opponentsBidHistory.get(sender).size()-1));
	} catch (Exception e) {
		return 0.0;
		}
    }
    ...
    public double getgap(Object sender) { try {
    	double gap = utilitySpace.getUtility(opponentsBidHistory.get(sender).get(opponentsBidHistory.get(sender).size()-1));
		gap = gap - utilitySpace.getUtility(opponentsBidHistory.get(sender).get(opponentsBidHistory.get(sender).size()-2));
		return gap;
	} catch (Exception e) {
		return 0.0;
		}
 ...
 // 交渉相手のn回前とその前の提案における自分の効用値の差
    public double getgapn(Object sender,int n) { try {
    	double gap = utilitySpace.getUtility(opponentsBidHistory.get(sender).get(opponentsBidHistory.get(sender).size()-1-n));
		gap = gap - utilitySpace.getUtility(opponentsBidHistory.get(sender).get(opponentsBidHistory.get(sender).size()-2-n));
		return gap;
	} catch (Exception e) {
		return 0.0;
		}
