...
public class RandomDance extends AbstractNegotiationParty {
	...
	private Map<String, PlayerDataLib> utilityDatas = new HashMap<>();
	private PlayerData myData = null;
	private int offerCount = 0;
	
	private List<String> nash = new LinkedList<String>();
	private Map<String,Bid> olderBidMap = new HashMap<>();
	...
	public Action chooseAction(List<Class> validActions) {
		...
		Map<String, PlayerData> utilityMap = new HashMap<>();
		...
		for(String string:olderBidMap.keySet()){
			...
			for(String player:utilityMap.keySet()){
				if(string.equals(player)){continue;}
	...
	public Map<String, Double> getWeights(){
		 ...
		 * プレイヤーウェイトの計算
		 */
		Map<String,Double> playerWeight = new HashMap<>();
	...
	public void receiveMessage(Object sender, Action action) {
		...
		if(sender.toString().equals("Protocol")){
			...
			playerNumber = (int)inform.getValue();
	...
	private boolean IsAccept(double target,double utility){
		...
		//targetより大きければAccept
			if(utility > target){
				//System.err.println("Accept utility over target! " + target + " " + utility);
				return true;
			}
		return false;
	}
	...
	private double GetTarget(Map<String,PlayerData> datas){
		...
		Bid bid = SearchBidWithWeights(datas, weights);
		try {
		...
		*/
		...
		//System.err.println("time = " + timeline.getTime()+ "target = " + target +" max = " + max);
		return target;
	}
	...
	private Bid SearchBid(double target ,Map<String, PlayerData> datas ,Map<String,Double> weights) throws Exception{
		...
		Map<String,Double> weightbuf = new HashMap<>();
		/*
		 * 敵ウェイトを合計1になるようにする
		 */
		double sum = 0;
...
class PlayerData{
	...
	Map<Issue,IssueData> map = new HashMap<>();
	Set<Bid> history = new HashSet<>();
	double derta = 1.00;
	...
	class IssueData{
		...
		private Map<Value, Double> map = new HashMap<>();
