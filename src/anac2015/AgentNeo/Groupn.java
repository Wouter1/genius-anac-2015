...
public class Groupn extends AbstractNegotiationParty {
    ...
    private Bid Maximum_Utility_Bid;//the bid with the maximum utility over the utility space.
	...
	public Groupn(UtilitySpace utilitySpace,
        ...
        this.utilitythreshold = utilitySpace.getUtility(Maximum_Utility_Bid); //initial utility threshold
	...
	private boolean AcceptOffer(Bid opponentBid, Bid ownBid) {
	        ...
	        if (currentUtility >= this.utilitythreshold || currentUtility >= nextRoundUtility) {
	            return true; //SCENARIO 1 Opponent's proposed utility surpasses threshold or Agent's next bid
	        } else {
	        	//SCENARIO 2
	            //if the opponent's proposed utility with discount is larger than the predicted 
	        	//maximum utility with discount, then accept it. It is the most the Agent could ever achieve.
	            ...
	            double bestMaxUtility = this.utilitySpace.getUtilityWithDiscount(bidOptions.getBestBidInHistory(), timeline);
	            //What the Agent could possibly achieve given what has bids have been proposed
	            ...
	            if (bestMaxUtility > predictMaxUtility){ 
	                try {
	                    ...
	                    if (utilitySpace.getUtility(opponentBid) >= utilitySpace.getUtility(bidOptions.getBestBidInHistory()) - 0.1 /*0.01*/) {
	                        return true;
	                    } else {
	...
	private Bid AgentOffer() {
		...
		case 1: bid = bidOptions.ChooseBid1(candidateBids, this.utilitySpace.getDomain());
		break;
		case 2: bid = bidOptions.ChooseBid2(candidateBids, this.utilitySpace.getDomain());
		break;
		case 3: bid = bidOptions.ChooseBid3(candidateBids, this.utilitySpace.getDomain());
		break;
		}
		return bid;	
    }
   ...
   private List<Bid> getBidsInRange(double lowerBound, double upperBound) {
    ...
    private boolean TerminateNegotiation(Bid ownBid) {
        ...
        if (currentUtility >= this.utilitythreshold || currentUtility >= nextRoundUtility) {
            ...
            if (currentMaxUtility > predictMaxUtility /* && timeline.getTime() > this.concedeToDiscountingFactor*/ ) {
                return true; //since the reservation value is going to be the most the Agent will get
            } else {
                return false;
            }
        }
	...
	public void receiveMessage(Object sender, Action action) {
		...
		if (ActionOfOpponent instanceof Offer){ //if Agent is not the first to act, update opponent's offer first
			this.bidOptions.addOpponentBid(((Offer) ActionOfOpponent).getBid(), utilitySpace.getDomain(), this.utilitySpace, IDOfOpponent);
