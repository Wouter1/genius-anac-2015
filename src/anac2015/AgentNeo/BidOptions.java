...
public class BidOptions {
	    ...
	    private double bid_maximum_utility_from_opponent3;//the bid with maximum utility proposed by the opponent so far.
	            ...
	            for (Issue lIssue : issues) {
	                switch (lIssue.getType()) {
	                    ...
	                    case DISCRETE:
	                       ...
	                       opponentBidsStatisticsDiscreteOpp1.add(discreteIssueValuesMap);
	                       opponentBidsStatisticsDiscreteOpp2.add(discreteIssueValuesMap);
	                       opponentBidsStatisticsDiscreteOpp3.add(discreteIssueValuesMap);
	                    ...
	                    case INTEGER:
	                        ...
	                        opponentBidsStatisticsForIntegerOpp1.add(numOfValueProposals);
	                        opponentBidsStatisticsForIntegerOpp2.add(numOfValueProposals);
	                        opponentBidsStatisticsForIntegerOpp3.add(numOfValueProposals);
	    ...
	    protected void addBidOpp1(Bid bid, UtilitySpace utilitySpace) {
	    		...
	    		bidHistory.add(bid);
            	if (bidHistoryOpp1.indexOf(bid) == -1) { //list does not contain the bid
	        ...
	        try {
	            if (bidHistoryOpp1.size() == 1) {
                    ...
                    this.bid_maximum_utility_from_opponent1 = utilitySpace.getUtility(this.bid_maximum_from_opponent1);

	            } else {
	                
	                    this.bid_maximum_from_opponent1 = bid;
	                    this.bid_maximum_utility_from_opponent1 = utilitySpace.getUtility(bid);
	              	            }
	        } catch (Exception e) {
	    ...
	    protected void addBidOpp2(Bid bid, UtilitySpace utilitySpace) {
	    	...
	    	bidHistory.add(bid);
        	if (bidHistoryOpp2.indexOf(bid) == -1) { //list does not contain the bid
        ...
        try {
            if (bidHistoryOpp2.size() == 1) {
                ...
                this.bid_maximum_utility_from_opponent2 = utilitySpace.getUtility(this.bid_maximum_from_opponent2);

            } else {
               
                    this.bid_maximum_from_opponent2 = bid;
                    this.bid_maximum_utility_from_opponent2 = utilitySpace.getUtility(bid);
	    ...
	    protected void addBidOpp3(Bid bid, UtilitySpace utilitySpace) {
	    	...
	    	bidHistory.add(bid);
        	if (bidHistoryOpp3.indexOf(bid) == -1) { //list does not contain the bid
        ...
        try {
            if (bidHistoryOpp3.size() == 1) {
                ...
                this.bid_maximum_utility_from_opponent3 = utilitySpace.getUtility(this.bid_maximum_from_opponent3);

            } else {
               
                    this.bid_maximum_from_opponent3 = bid;
                    this.bid_maximum_utility_from_opponent3 = utilitySpace.getUtility(bid);
	    ...
	    protected void addOpponentBid(Bid bidToUpdate, Domain domain, UtilitySpace utilitySpace, Object IDOfOpponent) {
	    ...
	    private void updateStatisticsOpp1(Bid bidToUpdate, boolean toRemove, Domain domain) {
	            try {
	                ArrayList<Issue> issues = domain.getIssues();
	                ...
	                for (Issue lIssue : issues) {
	                    ...
	                    Value v = bidToUpdate.getValue(issueNum); //v is the reference key
	                    switch (lIssue.getType()) {
	                        case DISCRETE:
	                            if (opponentBidsStatisticsDiscreteOpp1 == null) {
	                                System.out.println("opponentBidsStatisticsDiscreteOpp1 is NULL");
	                            } else if (opponentBidsStatisticsDiscreteOpp1.get(discreteIndex) != null) {
	                                int ValueofCounter = opponentBidsStatisticsDiscreteOpp1.get(discreteIndex).get(v);
	                                if (toRemove) {
	                                    ValueofCounter--;
	                                } else {
	                                    ValueofCounter++;
	                                }
	                                opponentBidsStatisticsDiscreteOpp1.get(discreteIndex).put(v, ValueofCounter);
	                            }
	                            discreteIndex++;
	                            break;
	                        ...
	                        case INTEGER:
	                            ...
	                            int countPerValue = opponentBidsStatisticsForIntegerOpp1.get(integerIndex).get(valueIndex);
	                            if (toRemove) {
	                                countPerValue--;
	                            } else {
	                                countPerValue++;
	                            }
	                            opponentBidsStatisticsForIntegerOpp1.get(integerIndex).set(valueIndex, countPerValue);
	                            integerIndex++;
	                            break;
						default:
							break;
	                ...
	                System.out.println("Exception in updateStatistics1: " + e.getMessage());
	            }
	        }
	
	    private void updateStatisticsOpp2(Bid bidToUpdate, boolean toRemove, Domain domain) {
            try {
                ArrayList<Issue> issues = domain.getIssues();
                    ...
                    switch (lIssue.getType()) {
                        case DISCRETE:
                            if (opponentBidsStatisticsDiscreteOpp2== null) {
                                System.out.println("opponentBidsStatisticsDiscreteOpp2 is NULL");
                            } else if (opponentBidsStatisticsDiscreteOpp2.get(discreteIndex) != null) {
                                int ValueofCounter = opponentBidsStatisticsDiscreteOpp2.get(discreteIndex).get(v);
                                if (toRemove) {
                                    ValueofCounter--;
                                } else {
                                    ValueofCounter++;
                                }
                                opponentBidsStatisticsDiscreteOpp2.get(discreteIndex).put(v, ValueofCounter);
                            }
                            discreteIndex++;
                            break;
                        ...
                        case INTEGER:
                            ...
                            int valueIndex = valueInteger - lIssueInteger.getLowerBound(); //For ex. LowerBound index is 0, and the lower bound is 2, the value is 4, so the index of 4 would be 2 which is exactly 4-2
                            int countPerValue = opponentBidsStatisticsForIntegerOpp2.get(integerIndex).get(valueIndex);
                            if (toRemove) {
                                countPerValue--;
                            } else {
                                countPerValue++;
                            }
                            opponentBidsStatisticsForIntegerOpp2.get(integerIndex).set(valueIndex, countPerValue);
                            integerIndex++;
                            break;
					default:
						break;
                ...
                System.out.println("Exception in updateStatistics2: " + e.getMessage());
            }
        }
	    private void updateStatisticsOpp3(Bid bidToUpdate, boolean toRemove, Domain domain) {
            try {
                ArrayList<Issue> issues = domain.getIssues();
                    ...
                    switch (lIssue.getType()) {
                        case DISCRETE:
                            if (opponentBidsStatisticsDiscreteOpp3 == null) {
                                System.out.println("opponentBidsStatisticsDiscreteOpp3 is NULL");
                            } else if (opponentBidsStatisticsDiscreteOpp3.get(discreteIndex) != null) {
                                int ValueofCounter = opponentBidsStatisticsDiscreteOpp3.get(discreteIndex).get(v);
                                if (toRemove) {
                                    ValueofCounter--;
                                } else {
                                    ValueofCounter++;
                                }
                                opponentBidsStatisticsDiscreteOpp3.get(discreteIndex).put(v, ValueofCounter);
                            }
                            discreteIndex++;
                            break;
                        ...
                        case INTEGER:
                            ...
                            int valueIndex = valueInteger - lIssueInteger.getLowerBound(); //For ex. LowerBound index is 0, and the lower bound is 2, the value is 4, so the index of 4 would be 2 which is exactly 4-2
                            int countPerValue = opponentBidsStatisticsForIntegerOpp3.get(integerIndex).get(valueIndex);
                            if (toRemove) {
                                countPerValue--;
                            } else {
                                countPerValue++;
                            }
                            opponentBidsStatisticsForIntegerOpp3.get(integerIndex).set(valueIndex, countPerValue);
                            integerIndex++;
                            break;
					default:
						break;
                ...
                System.out.println("Exception in updateStatistics3: " + e.getMessage());
            }
        }
	    ...
	    protected Bid getBestBidInHistory() {
	    	...
	    	if(token_counter.get("opponent 1") > token_counter.get("opponent 2") && token_counter.get("opponent 1") > token_counter.get("opponent 3")){
		    	
		        bid_maximum = this.bid_maximum_from_opponent1;

	    	}
	    	else if(token_counter.get("opponent 2") > token_counter.get("opponent 1") && token_counter.get("opponent 2") > token_counter.get("opponent 3")){
		        
	    		bid_maximum = this.bid_maximum_from_opponent2;

	    	}
	    	else if(token_counter.get("opponent 3") > token_counter.get("opponent 2") && token_counter.get("opponent 3") > token_counter.get("opponent 1")){
                ...
                for (int j = 0; j < issues.size(); j++) {
                    ...
                    switch (issues.get(j).getType()) {
                        ...
                        case INTEGER:
                            ...
                            int valueInteger = ((ValueInteger) v).getValue();
                            int valueIndex = valueInteger - lIssueInteger.getLowerBound(); //For ex. LowerBound index is 0, and the lower bound is 2, the value is 4, so the index of 4 would be 2 which is exactly 4-2
                            int ValueofCounter = opponentBidsStatisticsForIntegerOpp1.get(integerIndex).get(valueIndex);
                            MaximumCount += ValueofCounter;
                            integerIndex++;
                            break;
                    }
                }
                if (MaximumCount > MaximumFreq) {//choose the bid with the maximum MaximumCount
                    MaximumFreq = MaximumCount;
                    maxIndex = i;
                } else if (MaximumCount == MaximumFreq) {//random exploration
                ...
                for (int j = 0; j < issues.size(); j++) {
                    ...
                    switch (issues.get(j).getType()) {
                        ...
                        case INTEGER:
                            ...
                            int valueInteger = ((ValueInteger) v).getValue();
                            int valueIndex = valueInteger - lIssueInteger.getLowerBound(); //For ex. LowerBound index is 0, and the lower bound is 2, the value is 4, so the index of 4 would be 2 which is exactly 4-2
                            int ValueofCounter = opponentBidsStatisticsForIntegerOpp2.get(integerIndex).get(valueIndex);
                            MaximumCount += ValueofCounter;
                            integerIndex++;
                            break;
                    }
                ...
                for (int j = 0; j < issues.size(); j++) {
                    ...
                    switch (issues.get(j).getType()) {
                        ...
                        case INTEGER:
                            ...
                            int valueInteger = ((ValueInteger) v).getValue();
                            int valueIndex = valueInteger - lIssueInteger.getLowerBound(); //For ex. LowerBound index is 0, and the lower bound is 2, the value is 4, so the index of 4 would be 2 which is exactly 4-2
                            int ValueofCounter = opponentBidsStatisticsForIntegerOpp3.get(integerIndex).get(valueIndex);
                            MaximumCount += ValueofCounter;
                            integerIndex++;
                            break;
                    }
