...
public class negotiatingInfo {		
    ...
    public void updateInfo(Object sender, Bid offeredBid) {    	
    	try { updateNegotiatingInfo(sender, offeredBid); } // �����̍X�V 
    	catch (Exception e1) {
    ...
    private void initValueRelativeUtility() throws Exception{
		...
		for(Issue issue:issues){ 
			...
			for(Value value:values){ valueRelativeUtility.get(issue).put(value, 0.0); } // �_�_�s�̗v�f�̏���
    	}
	}
    ...
    public void updateNegotiatingInfo(Object sender, Bid offeredBid) throws Exception {
    	...
    	opponentsPowSum.put(sender, opponentsPowSum.get(sender) + Math.pow(util, 2)); // ���a
    	
    	int round_num = opponentsBidHistory.get(sender).size();
    	opponentsAverage.put(sender, opponentsSum.get(sender) / round_num); // ����
    	opponentsVariance.put(sender, (opponentsPowSum.get(sender) / round_num) - Math.pow(opponentsAverage.get(sender), 2)); // ���U
    	
    	if(opponentsVariance.get(sender) < 0){opponentsVariance.put(sender, 0.0);}
    	opponentsStandardDeviation.put(sender, Math.sqrt(opponentsVariance.get(sender))); // �W���΍�
    }
    ...
    // �^�����ǂ���(�Z�b�g)
    public void setOpponentsBool(Object sender,boolean bool) { opponentsBool.put(sender, bool); return;}
