...
import anac2015.Forseti.OpponentModel;
...
public class NegotiationModel {
	...
	private UtilitySpace baseUtilSpace; // A standardized utility space with all weights and utilities equal.
	private UtilitySpace myUtilSpace;
	private HashMap<AbstractNegotiationParty,OpponentModel> opponentModels = new HashMap<AbstractNegotiationParty,OpponentModel>(); // One model per opponent.
	...
	public double getEstimatedSocialWelfare(Bid bid) {
		...
		for (AbstractNegotiationParty o : opponents) {
			socialWelfare = socialWelfare + opponentModels.get(o).getUtility(bid);
		}
		return socialWelfare;
	}
	
	// Returns true if the new bid is worse than the last bid for all participants.
