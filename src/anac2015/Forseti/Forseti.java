...
public class Forseti extends AbstractNegotiationParty {
	...
	private AbstractNegotiationParty currentAgent = null; // The agent currently bidding/accepting
	private NegotiationModel model = new NegotiationModel(getUtilitySpace()); // Our model of the negotiation.
	...
	private List<Bid> goodBids = new ArrayList<Bid>(); // List of bids with utility above our utility cutoff value
	private Bid bestSocialBid; // The bid that maximizes the Nash product, according to the current model.
	private int round = 1;
	private double timeFactor = 0.05; // Used for discounting the reservation value
	private Random rng;
	 ...
	 */
	public Forseti(UtilitySpace utilitySpace,
		...
		for (int i = 0; i < utilitySpace.getNrOfEvaluators(); i++) {
			...
			if (eva.getType() == EVALUATORTYPE.DISCRETE) {
				noOfPossibleBids = noOfPossibleBids * ((EvaluatorDiscrete) eva).getValues().size();
			}
			else if (eva.getType() == EVALUATORTYPE.INTEGER) {
				EvaluatorInteger evai = (EvaluatorInteger) eva;
				noOfPossibleBids = noOfPossibleBids * (evai.getUpperBound() - evai.getLowerBound());
			}
		}
	...
	public Action chooseAction(List<Class> validActions) {
		...
		double utilityOfLastBid = utilitySpace.getUtilityWithDiscount(lastBid, timeline);
		...
		// For the rest of the negotiation: Offer the bid with the highest estimated Nash product. 
		// However, don't offer the same bid two times in a row. Also don't offer a bid that is 
		// unfortunate (worse for all participants than the last bid) according to the model.
		else {
			updateBestSocialBid();
	...
	// Finds (almost) all bids with utility above the cutoff and stores the in the goodBids list.
	// The size of the list should be within 5% to 20% of the total number of bids; if this is not the case,
	...
	private void findGoodBids (double cutoff) {
		...
		// If the total number of bids is relatively low, we need a finer adjustment
		...
		// If the size of the list is not within 5% to 20% of the total number of bids,
	...
	// Finds the bid with highest estimated social welfare and stores it in the bestSocialBid variable.
	...
	private double getReservationValue() {
		return utilityCutoff - utilityCutoff*Math.pow((timeline.getTime()),(1/timeFactor));
	}
	...
	public void receiveMessage(Object sender, Action action) {
		...
		// Add the bid to the model. If the message is an accept, add the accepted bid.
		if (!noSender) {
